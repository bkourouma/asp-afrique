...
const blogRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/', async (request, reply) => {
    try {
      ...
      if (status && status !== 'all') {
        ...
        baseConditions.status = 'published';
      }
      ...
      // Si on a une recherche ET d'autres conditions, utiliser AND pour combiner
      if (search) {
        ...
        if (Object.keys(baseConditions).length > 0) {
          where.AND = [
            ...Object.entries(baseConditions).map(([key, value]) => ({ [key]: value })),
            searchCondition,
          ];
        } else {
        ...
        // Pas de recherche, utiliser les conditions de base directement
      ...
      fastify.log.error('Error fetching blog articles:', {
  ...
  fastify.get('/:id', async (request, reply) => {
    try {
      ...
      fastify.log.error('Error fetching blog article by ID:', {
        error: error?.message || String(error),
        stack: error?.stack,
        code: error?.code,
      });
  ...
  fastify.get('/by-slug/:slug', async (request, reply) => {
    try {
      ...
      fastify.log.error('Error fetching blog article by slug:', {
        error: error?.message || String(error),
        stack: error?.stack,
        code: error?.code,
      });
